...
function validator (formSelector) {
    ...
    var formRules = {
        ...
        confirm: function(value){
            return value === formValidator.querySelector('[name="newpassword"]').value ? undefined : 'Mật khẩu không khớp.'
        },
    ...
    function getParent (selection) {
        while (selection.parentElement) {
            if (selection.parentElement.matches('.form-input')) {
    ...
    function handleValidate(input) {
        ...
        var errorMessage;
        // nếu tồn tại rule của input thì mới lặp
        if (ruleOfinput) {
    ...
    for (var input of inputs) {
        ...
        // nếu input được gắn rules thì mới chạy qua
        if (rules) {
